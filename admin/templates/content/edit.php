...
<div class="row">
	<div class="row-data">
		<form role="form" action="<?= $this->url('content')?>" method="post" enctype="multipart/form-data">
			<?php Form::printSelect('pid', _t('Parent page'), 'treeselect', isset($this->data['pid']) ? $this->data['pid'] : 0, $this->pages, 'pid') ?>
			<?php Form::printField('link', _t('Page URL'), 'text', isset($this->data['link']) ? $this->data['link'] : '', '', '', ['prefix' => DOMAIN . '/']) ?>
			<?php Form::printSelect('gal_id', _t('Assigned gallery'), 'treeselect', isset($this->data['gal_id']) ? $this->data['gal_id'] : 0, $this->galleries) ?>
			<?php Form::printSelect('template', _t('Page template'), 'select', isset($this->data['template']) ? $this->data['template'] : '', $this->templates) ?>
			<?php Form::printField('image', _t('Page image'), 'file') ?>
			<?php if($this->data['image']) :?>
			<a href="/images/content/<?= $this->data['image'] ?>" target="_blank"><img src="/images/content/thumb_<?= $this->data['image'] ?>"></a>
			<?php endif; ?>
